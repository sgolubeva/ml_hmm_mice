...
def generate_data_psytrack(ntrials):
    ...
    simData = psy.generateSim(K=num_weights, N=ntrials, hyper=hyper,
...
def glmhmm_with_psy_cho(synth_psy_data):
    ...
    glmhmm_dyn_weights = get_glmhmm_dinmc_weights(new_glmhmm, true_glmhmm, glm_hmm_inpts, glmhmm_data_choices)
    fig, axes = plt.subplots(nrows=1, figsize=(20, 10), dpi=80, facecolor='w', edgecolor='k')
    #import ipdb; ipdb.set_trace()

    plot_dinamic_weights(axes, glmhmm_dyn_weights, (wMode)*-1)
    plt.tight_layout()
...
if __name__ == "__main__":
    ...
    synth_psy_data = generate_data_psytrack(ntrials) # generate data using psytrack
